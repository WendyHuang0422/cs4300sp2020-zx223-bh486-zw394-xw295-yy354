...
def retrieve_tweets(N,celeb_name,inc_retweets,api):
  ...
  print(N)
  pool = list(tweepy.Cursor(api.user_timeline,screen_name = celeb_name,tweet_mode='extended',count=200).items())
  print(pool)
  result = []
  count = 0
  fetched = set()
...
def twittter_aggregated(N,pool_size,celeb_name,inc_retweets):
  ...
  count = 0
  recent,idx_time = tweets_crawl(N,pool_size,celeb_name,api,inc_retweets)
  for idx in recent:
    idx = idx[0]._json
    temp = {}
...
def news_Aggregated(N,date1,date2,order,query=None):
  ...
  json
  """
  keyss = ['2036183222ab402cbce56ee293728fb1','8ba8b6889b1343508e1ba0b55849ec31']
    ...
    for sent in sentences:
      ...
      if pos:
        if nltk1:
          ...
          sent_tok = [sent_tok[i] for i in range(len(sent_tok)) if tags[i][:1] in wanted]
        elif not nltk1:
          tags = [word.pos_ for word in rich_word]
          sent_tok = [sent_tok[i] for i in range(len(sent_tok)) if tags[i] in wanted_scp]
      elif stemming:
...
def idf_topic_cluster(inv_idx,tweet1,tok_tweet_list,idf,doc_norms):
  ...
  top = scores[0][0]
  result = [(tok_tweet_list[record[1]],record[1]) for record in scores if record[0]>1/4*top]
  ...
  for length in range(len(list_keywords),int(0.4*len(list_keywords)),-1):
    ...
    for news_source in ['News','Guardian']:
      ...
      if news_source == 'Guardian':
        ...
        result.extend(news_Aggregated(10,date1,date2,order='relevancy',query=query))
    if len(result)>0:
      print(length,combo)
      return result
  return result
...
def keywords_per_tweet(list_of_tweets,idx_time,N_tweets,N_keyword,nltk1):
  ...
  """
  tokenized,tweetList_raw = tokenize_tweets(list_of_tweets,stemming=False,pos=True,lower=True,remove_stop=True,nltk1=nltk)
...
def db_news_retrieval(list_of_tweets,N_news,input_keys,N_tweets):
  ...
  tokenized,tweetList_raw = tokenize_tweets(list_of_tweets,stemming=False,pos=True,lower=True,remove_stop=True,nltk1=True)
...
def full_text_integerate(list_dictionaries):
  ...
  tokenized = tokenize_news(full_text,stemming=False,pos=True,lower=True,remove_stop=True,nltk1=False)
